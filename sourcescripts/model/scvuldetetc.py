...
class LitSvulDetGAT(LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        pred_node = (node_probs[:, 1] > 0.3).long()
    ...
    def test_step(self, batch, batch_idx):
        ...
        func_label = batch.ndata['_FVULN'][0].long()
        node_probs = torch.softmax(node_logits, dim=1)
        node_preds = (node_probs[:, 1] >= 0.3).long()
...
if __name__ == '__main__':
    ...
    config_grid = {
        ...
        'max_epochs': 2, # 5, 3, #2, #10,  3 is good for now
        'in_feats': 768, #768, #384, #100 must e the same as the feature in graph 
        'check_patience': 2, # 10 work well for full data # 2, # 2, (10 is work correctly with all data)
        'batch_size':  1, # dataloader
        'num_heads': 4,
        'check_monitor': 'val_f1', # val_acc, 'val_f1' one of these
        'hidden_feats': [ 64], # , [64, 256]
        'lr': [5e-5], # [5e-5, 1e-4, 2e-4, 5e-4],
        'dropout': [0.4],
        # gl paramts
        # "embedd_method": "Codebert",
