    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.style == "methodlevel":
            ...
            if (node_labels == 1).sum() > 0:
                ce_node = nn.CrossEntropyLoss(reduction='none')
                loss_per_node = ce_node(node_logits, node_labels)
                weights = torch.where(node_labels == 1, 1.0, 0.6).to(node_logits.device)
                node_loss = (loss_per_node * weights).mean()
                entropy = self.loss_fn.entropy_loss(node_logits)
            else:
                node_loss = torch.tensor(0.0, device=node_logits.device, requires_grad=True)
...
"""
The results reported are from different runs to modify the learning process and adjust the settings. 
The following configurations performed best for the current dataset. 
To train on a different dataset, you may consider increasing other values for optimal parameter search.
"""
